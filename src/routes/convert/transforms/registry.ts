// Pure transform registry — no Svelte or DOM dependencies, safe to import
// from web workers and node tests. UI-only concerns (options components)
// are attached in index.ts, main thread only.
...
export const defaultOpts = Object.fromEntries(
  Object.entries(allTransforms).map(([key, value]) => [key, value.defaults])
)

export function analyze(src: Step, options: Record<string, string | undefined>): AnalyzeResult[] {
  const results = Object.entries(allTransforms)
